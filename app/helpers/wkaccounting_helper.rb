	...
	include WktimeHelper
		...
		def getLedgerTypeHash
			ledgerType = {
            	...
            	"Capitaux propres "=> "Capitaux propres",
				"Bénéfices non répartis / déficit"=> "Bénéfices non répartis / déficit",
            	"Bénéfices non répartis / déficit "=> "Bénéfices non répartis / déficit",
            	...
            	"Dépenses agricoles" => "Dépenses agricoles",
            	"Éléments extra/impôts sur le revenu" => "Éléments extra/impôts sur le revenu"
		...
		def getLedgerIGRFTypeHash
			ledgerIGRFType = {
				"Actifs" => ["Actifs à court terme","Immobilisations","Actifs à long terme"],
				"Passifs" => ["Passifs à court terme","Passifs à long terme"],
				"Capitaux propres" =>["Capitaux propres "],
				"Bénéfices non répartis / déficit" => ["Revenus", "Coût des ventes", "Frais d'exploitation", "Revenus agricoles", "Dépenses agricoles", "Éléments extra/impôts sur le revenu"]
			}
			ledgerIGRFType
		end
	
		def getLedgerIGRFRange
			ledgerIGRFTypeRange = {
				"Actifs à court terme" => [1000,1599], #'SH',
				"Immobilisations" => [1600,2179],
				"Actifs à long terme" => [2180,2599],
				"Passifs à court terme" => [2600,3139],
				"Passifs à long terme" => [3140,3499],
				"Capitaux propres " => [3500,3640],
				"Bénéfices non répartis / déficit " => [3660,3849],
            	"Revenus" => [7000,8259],
            	"Coût des ventes" => [8300,8519],
            	"Frais d'exploitation" => [8520,9369],
            	"Revenus agricoles" => [9370,9659],
            	"Dépenses agricoles" => [9660,9970],
            	"Éléments extra/impôts sur le revenu" => [9975,9999]
			}
			ledgerIGRFTypeRange
		end
	
		def sourceOfFundLTypesIGRF
			["Passifs"]
		end
	
		def appOfFundLTypes
			 ["Actifs","Capitaux propres","Bénéfices non répartis / déficit"]
		end
		
		def incomeLedgerTypesIGRF
			["Revenus","Revenus agricoles"]
		end
		
		def expenseLedgerTypesIGRF
			["Coût des ventes", "Frais d'exploitation","Dépenses agricoles", "Éléments extra/impôts sur le revenu"]
		end
		...
		def getSubEntriesIGRF(from, asOnDate, ledgerType)
			subEntriesHash = nil
			bsEndDate = ledgerType == 'Bénéfices non répartis / déficit' ? from -1  : asOnDate
			...
			#end
			if ledgerType == 'Bénéfices non répartis / déficit'
				totalIncome = 0
				totalExpense = 0
				defLedger = WkLedger.where(:igrf_account_number => '3680')
				incomeLedgerTypesIGRF.each do |type|
					totalIncome = totalIncome + getEntriesTotal(subEntriesHash[type])
				end
				expenseLedgerTypesIGRF.each do |type|
					totalExpense = totalExpense + getEntriesTotal(subEntriesHash[type])
				end
				subEntriesHash.clear
				plOpeningBal = getEachLedgerBSAmtIGRF(bsEndDate, 'Bénéfices non répartis / déficit').blank? ? 0 : getEachLedgerBSAmtIGRF(bsEndDate, 'Bénéfices non répartis / déficit').values[0]
				subEntriesHash[l(:wk_label_opening)+ " " + l(:wk_field_balance)] = totalIncome - totalExpense + (defLedger[0].opening_balance.blank? ? 0 : defLedger[0].opening_balance) + plOpeningBal
				subEntriesHash[l(:label_current)+ " " + l(:label_period)] = getPLforIGRF(from, asOnDate)
		...
		def getPLforIGRF(from, to)
			totalIncome = 0
			totalExpense = 0
			incomeLedgerTypesIGRF.each do |type|
				income = getEachLedgerSumAmtIGRF(from, to, type)
				totalIncome = totalIncome + income.values.inject(:+) unless income.blank?
			end
			
			expenseLedgerTypesIGRF.each do |type|
				expense = getEachLedgerSumAmtIGRF(from, to, type)
				totalExpense = totalExpense + expense.values.inject(:+) unless expense.blank?
			end
			profit = totalIncome - totalExpense
			profit
		...
		def getEachLedgerBSAmt(asOnDate, ledgerType)
			...
			balHash = Hash.new
			ledgers = WkLedger.where(:ledger_type => ledgerType).order(:name)
			ledgers.each do |ledger|
				unless profitHash[ledger.id].blank? && (ledger.opening_balance.blank? || ledger.opening_balance == 0)
            		...
            		unless ledger.igrf_account_number.blank?
            			key = ledger.name + " (" + ledger.igrf_account_number.to_s + ")"
                    else
                    	key = ledger.name
                	end
					balHash[key] = (profitHash[ledger.id].blank? ? 0 : profitHash[ledger.id]) + ((ledger.opening_balance.blank? || ledger.igrf_account_number == '3680') ? 0 : ledger.opening_balance)
				end
			end
			balHash
		end
		def getEachLedgerBSAmtIGRF(asOnDate, ledgerType)
        	balHash = Hash.new
			typeArr = ['c', 'd']
			detailHash = Hash.new
			ledgerIgrfrange = getLedgerIGRFRange[ledgerType]
          	unless ledgerIgrfrange.blank?
				typeArr.each do |type|
					detailHash[type] = 
					WkGlTransactionDetail
						.includes(:ledger, :wkgltransaction)
						.where('wk_gl_transaction_details.detail_type = ? and wk_ledgers.igrf_account_number BETWEEN ? and ? and wk_gl_transactions.trans_date <= ?', type, ledgerIgrfrange[0], ledgerIgrfrange[1], asOnDate)
						.references(:ledger,:wkgltransaction)
						.group('wk_ledgers.igrf_account_number')
						.sum('wk_gl_transaction_details.amount')
            	end

			profitHash = calculateBalanceIGRF(detailHash['c'], detailHash['d'], ledgerType)

			ledgers = WkLedger.where('wk_ledgers.igrf_account_number BETWEEN ? and ?', ledgerIgrfrange[0], ledgerIgrfrange[1]).order(:igrf_account_number)
			ledgers.each do |ledger|
		...
		def calculateBalanceIGRF(creditHash, debitHash, ledgerType)
			if isSubtractCrIGRF(ledgerType)
				creditHash = inverseHashVal(creditHash)
			else
				debitHash = inverseHashVal(debitHash)
			end
		...
		def isSubtractCrIGRF(ledgerType)
			isSubtract = true
			subLedTypeArr = []
			sourceOfFundLTypesIGRF.each do |val|
				subLedTypeArr = subLedTypeArr + getLedgerIGRFTypeHash[val] unless getLedgerIGRFTypeHash[val].blank?
			end
			subLedTypeArr = sourceOfFundLTypesIGRF + subLedTypeArr
			subLedTypeArr = subLedTypeArr + incomeLedgerTypesIGRF
		...
		def getEachLedgerSumAmtIGRF(from, to, ledgerType)
			typeArr = ['c', 'd']
			detailHash = Hash.new
        	ledgerIgrfrange = getLedgerIGRFRange[ledgerType]
			if ledgerType.blank?
				typeArr.each do |type|
					detailHash[type] = WkGlTransactionDetail.includes(:ledger, :wkgltransaction).where('wk_gl_transaction_details.detail_type = ? and wk_gl_transactions.trans_date between ? and ?', type, from, to).references(:ledger,:wkgltransaction).group('CONCAT(wk_ledgers.igrf_account_number, " - ", wk_ledgers.igrf_account_description)').sum('wk_gl_transaction_details.amount')
				end
			else
				typeArr.each do |type|
					detailHash[type] = WkGlTransactionDetail.includes(:ledger, :wkgltransaction).where('wk_gl_transaction_details.detail_type = ? and wk_ledgers.igrf_account_number BETWEEN ? and ? and wk_gl_transactions.trans_date between ? and ?', type, ledgerIgrfrange[0], ledgerIgrfrange[1], from, to).references(:ledger,:wkgltransaction).group('CONCAT(wk_ledgers.igrf_account_number, " - ", wk_ledgers.igrf_account_description)').sum('wk_gl_transaction_details.amount')
				end
			end
			profitHash = calculateBalanceIGRF(detailHash['c'], detailHash['d'], ledgerType)
			profitHash
