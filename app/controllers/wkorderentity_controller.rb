...
class WkorderentityController < WkbillingController
	unloadable
	  ...
	  def update
		  ...
		  unless params["invoice_id"].blank?
			  @invoice = WkInvoice.find(params["invoice_id"].to_i)
			  @invoice.invoice_date = params[:inv_date] unless params[:inv_date].blank?
			  if !api_request? || !params['invoiceItems'].blank?
		  ...
		  @invoice.gl_transaction_id = params[:gl_transaction_id] unless params[:gl_transaction_id].blank?
